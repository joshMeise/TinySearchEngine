...
int main(int argc, char *argv[]) {
	// Variable declarations.
	webpage_t *pageLoad;
	char *word, name[] = "outputFile", readWord[50], fname[50];
	...
	while (access(fname, R_OK) == 0) {
		...
		if ((ifile = fopen(name, "w")) == NULL)
			printf("File not opened corrrectly.\n");
		
		// Check that file can be opened for writing.
		if (access(name, W_OK) != 0)
			printf("File not accessible for writing.\n");
		...
		// Check to see if file is accessible for reading.
		if (access(name, R_OK) != 0)
			printf("File not accessible for reading.\n");
		...
		while ((fscanf(ifile, "%s", readWord)) != EOF) {
			...
			if (fgetc(ifile) != '\n')
				printf("Reading an actual character %d.\n", curID);
