...
static void printWordH(void *data) {
	wordQ_t *wrd = (wordQ_t *)data;
	printf("The word %s occurs in", wrd->word);
...
int main(int argc, char *argv[]) {
	// Variable declarations.
	webpage_t *pageLoad;
	char *word, name[] = "outputFile", readWord[50], *str = NULL;
	int32_t pos, docID;
	FILE *ifile;
	wordQ_t *wordQueue;
	hashtable_t *hash;
	...
	// Load webpage at given indx.
	pageLoad = pageload(docID, "../crawler/page");
	...
	while ((fscanf(ifile, "%s", readWord)) != EOF) {
		...
		// If word is already in hash table.
			...
			// Find the element in the word's queue associated with the given docID.
			// If the document ID is not already in the queue.
			if ((dc = (docCount_t *)qsearch(wordQueue->qp, searchQueue, (void *)&docID)) == NULL) {
				// Create a new document/count pair.
				dc = (docCount_t *)malloc(sizeof(docCount_t));
